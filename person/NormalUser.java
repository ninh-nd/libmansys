...
//	}
	...
	public static void SuccessfulNotification(ArrayList<Integer> book_id) {
		try(Connection conn = DatabaseManagement.connect()){
			...
			for(int id:book_id) {
				...
				ResultSet rs = stmt.executeQuery();
				while(rs.next()) {
					message = message + String.valueOf(no) + ". Book ID: " + String.valueOf(id) + " - " + rs.getString("title") + "\n";
				}
				no++;
			}
			JOptionPane.showMessageDialog(null, message.trim(), null, JOptionPane.PLAIN_MESSAGE);			
	...
	public static void rentBook(ArrayList<Integer> book_id, User user) {
		...
		if(book_id.size()+cur_books>MAXIMUM_NUMBER_OF_BOOKS) {
			JOptionPane.showMessageDialog(null, "You can only rent up to "+(MAXIMUM_NUMBER_OF_BOOKS-cur_books)+" books", null, JOptionPane.PLAIN_MESSAGE);
		}else {
			try(Connection conn = DatabaseManagement.connect();){
				...
				else FailedNotification(rented_id);
			} catch (SQLException e) {
				System.out.println(e.getMessage());
				e.printStackTrace();
			}
	...
	public static void returnBook(ArrayList<Integer> book_id, User user) {
	...
	public static void renewBook(ArrayList<Integer> book_id) {
		try(Connection conn = DatabaseManagement.connect();){
			...
			for(int id:book_id) {
				...
				ResultSet rs = stmt.executeQuery();
				while(rs.next()) {
					if(rs.getBoolean("is_extended")) extended_id.add(id);
					else {
					Date due_date = Date.valueOf(LocalDate.now().plusDays(30));
